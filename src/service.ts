...
import { LabnanaClient, ImageGenerationParams, ImageWithImageParams } from './client'

const logger = new Logger('labnana-draw-service')

export class DrawService {
  private client: LabnanaClient
  private ctx: Context
   ...
   * 处理文生图命令
   */
  async handleTextToImage(session: Session, prompt: string, options: any = {}) {
   ...
   * 处理图生图命令（文带图）
   */
  async handleImageToImage(session: Session, prompt: string, imageUrl: string, options: any = {}) {
    const startTime = Date.now()
    ...
    try {
      ...
      // 构建参数
      const params: ImageWithImageParams = {
        prompt,
        imageUrl,
        negativePrompt: options.negativePrompt,
        width: options.width || 512,
