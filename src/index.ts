...
export const name = 'labnana-draw'
...
export function apply(ctx: Context, config: Config) {
  // 创建Labnana客户端
  const client = new LabnanaClient(ctx, {
  ...
  // 创建绘图服务
  const drawService = new DrawService(ctx, client)

  // 辅助函数：根据比例和分辨率计算宽高
  function calculateDimensions(ratio: string, resolution: string): { width: number, height: number } {
    // 查找分辨率基准值
  ...
  ctx.command('图文混合 <prompt:text> [model:text] [ratio:text] [resolution:text]', 'AI图文混合（需要附带图片）')
    ...
    .action(async ({ session, options }, prompt, model = 'gemini-3-pro-image', ratio = '1:1', resolution = '1k') => {
      ...
      // 从消息中提取图片
      const imageUrl = session.elements?.find((el: any) => el.type === 'image')?.data?.url
  ...
  ctx.command('AI绘图', '交互式AI绘图向导')
    ...
    .action(async ({ session }) => {
      ...
      // 解析模型选择
      let selectedModel: string = MODELS[0].name // 默认
      ...
      if (!isNaN(modelNum) && modelNum >= 1 && modelNum <= MODELS.length) {
        ...
        const matched = MODELS.find(m => 
          modelResponse.toLowerCase().includes(m.name.toLowerCase()) ||
      ...
      // 解析分辨率选择
      let selectedResolution: string = RESOLUTIONS[0].value // 默认
      ...
      if (!isNaN(resNum) && resNum >= 1 && resNum <= RESOLUTIONS.length) {
        ...
        // 尝试匹配值
        const matched = RESOLUTIONS.find(r => 
          resResponse.toLowerCase().includes(r.value) ||
      ...
      if (isImageToImage) {
        ...
        // 等待图片消息
        const imageMessage = await new Promise<any>((resolve) => {
